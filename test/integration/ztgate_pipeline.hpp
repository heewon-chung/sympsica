...
// ztgate_pipeline — W2.1's mask-and-key pipeline: end-to-end generation of
// ZtGates (the offline correlated randomness the online z-test consumes).
//
// This is the exact code Setup will call at Phase 5. Per the plan's ownership
// note ("pipeline code lives in test form; moves to setup.cpp at Phase 5") it
// lives under test/integration/ for now, but it is written as a reusable
// library — no gtest dependency, no inline TEST() bodies — so the Phase-5 move
// is a file move plus a namespace change, not a rewrite.
//
...
// libOTe's type-erased socket handle: coproto::LocalAsyncSocket (used by this
// task's in-process tests) and coproto's asio/TCP socket (Task 8 / W2.4) both
// convert into it, so no local socket type is baked into the API.
//
