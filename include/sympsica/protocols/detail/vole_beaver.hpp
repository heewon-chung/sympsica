#ifndef SYMPSICA_PROTOCOLS_DETAIL_VOLE_BEAVER_HPP
#define SYMPSICA_PROTOCOLS_DETAIL_VOLE_BEAVER_HPP

// protocols/detail/vole_beaver — W2.2's noisy-VOLE-based OLE and
// Beaver-triple assembly, built on oc::NoisyVoleSender/Receiver<Fp,Fp,
// CoeffCtxFp61> (vendor/libOTe/libOTe/Vole/Noisy/*.h). Companion to
// ztgate_pipeline.{hpp,cpp} (W2.1): same OtPool/Role types (reused from
// ztgate_pipeline.hpp), same coproto::Socket&-based two-party pattern.
//
// MOVED to production (task-16-brief.md, W5.1, R-HOST consolidation): this
// file originated under test/integration/ (task-6, W2.2); Phase 5's Setup
// (protocols/setup.cpp) is the production caller, so this is now the ONE
// production copy — test/integration/vole_beaver.hpp is a thin forwarding
// shim so every existing Phase 2-4 test call site keeps compiling unchanged.
//
...
#endif // SYMPSICA_PROTOCOLS_DETAIL_VOLE_BEAVER_HPP
